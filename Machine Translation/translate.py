	...
	for tok1 in tokens1:
		for tok2 in tokens2:
			try:
				...
				bigrams[tok1][tok2] = 1
# By this point we should have the cooccurence counts in the 'bigrams' matrix

dictionary = {}
# For each word in lang 1
for word1 in bigrams:
	bestWord = ' '
	maxcount = 0
# For each translation of the word in lang 1
	for word2 in bigrams[word1]:
    # If it's count is higher than the current maximum, set a new best translation
		if bigrams[word1][word2] > maxcount:
			bestWord = word2
			maxcount = bigrams[word1][word2]
	dictionary[word1] = bestWord

str3 = sys.argv[3]
formatted = str3.split(' ')
